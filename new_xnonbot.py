...
`$help` - Send the list of available commands with their descriptions
...
All commands, except those starting with "$" are prompted using the built-in slash command on discord.
"""
...
async def on_ready():
    ...
    try: 
        synced = await bot.tree.sync() # Syncs all available slash commands as it is a discord requirement
...
@bot.tree.command(name="about", description="Send the information about this bot.")
async def hello(interaction: discord.Interaction):
    await interaction.response.send_message("As the name implies, I'm a simple chatbot created by XnonXte!")


@bot.tree.command(name="roll", description="Choose a random dice roll (1 from 6).")
async def hello(interaction: discord.Interaction):
    await interaction.response.send_message(random.randint(1, 6))


@bot.tree.command(name="github", description="Send the github page for this bot.")
async def hello(interaction: discord.Interaction):
    await interaction.response.send_message("https://github.com/XnonXte/XnonBot")


@bot.tree.command(name="quote", description="Get a random quote from zenquote.io.")
async def quote(interaction: discord.Interaction):
    quote = BotHTTPRequests.get_quote()
    await interaction.response.send_message(quote)


@bot.tree.command(name="dog", description="Get a random dog picture from dog.ceo.")
async def dog(interaction: discord.Interaction):
    dog = BotHTTPRequests.get_dog_pic()
    await interaction.response.send_message(dog)


@bot.tree.command(name="cat", description="Get a random cat picture from api.thecatapi.com.")
async def cat(interaction: discord.Interaction):
    cat = BotHTTPRequests.get_cat_pic()
    await interaction.response.send_message(cat)


@bot.tree.command(name="waifu", description="Get a random waifu picture from api.waifu.pics (it's sfw don't worry!)")
...
async def rps(interaction: discord.Interaction, choice: str):
    ...
    if choice not in choices:
      await interaction.response.send_message('Invalid choice. Please choose either rock, paper, or scissors!', ephemeral=True)
      return

    if choice == bot_choice:
      await interaction.response.send_message(f'{interaction.user.mention} chose {choice}. I chose {bot_choice}. We tied!')
    elif choice == 'rock' and bot_choice == 'scissors':
      await interaction.response.send_message(f'{interaction.user.mention} chose {choice}. I chose {bot_choice}. {interaction.user.mention} won!')
    elif choice == 'scissors' and bot_choice == 'paper':
      await interaction.response.send_message(f'{interaction.user.mention} chose {choice}. I chose {bot_choice}. {interaction.user.mention} won!')
    elif choice == 'paper' and bot_choice == 'rock':
      await interaction.response.send_message(f'{interaction.user.mention} chose {choice}. I chose {bot_choice}. {interaction.user.mention} won!')
    else:
      await interaction.response.send_message(f'{interaction.user.mention} chose {choice}. I chose {bot_choice}. I won!')
...
async def pexels(interaction: discord.Interaction, query: str):
    ...
    await interaction.response.send_message(f"An image of {query} has been generated! Photographed by: {image_output[0]}, original link: {image_output[2]} - Powered by pexels.com")

@bot.event
async def on_message(message):
    def check_message(initial_requests):
        return initial_requests.author == message.author and initial_requests.channel == message.channel and initial_requests.content.lower() in ["true", "false"]
    ...
    if message.content.startswith("$animaltrivia"):
        ...
        await message.channel.send(html.unescape(f"{animal_trivia[0]} True or false? The difficulty is {animal_trivia[1]}."))
        ...
        if response.content.lower() == animal_trivia[2].lower():
            ...
            await message.channel.send(f"Sorry, but the correct answer was {animal_trivia[2]}.")

    if message.content.startswith("$mathtrivia"):
        math_trivia = BotHTTPRequests.get_math_trivia()
        await message.channel.send(html.unescape(f"{math_trivia[0]} True or false? The difficulty is {math_trivia[1]}."))
        ...
        if response.content.lower() == math_trivia[2].lower():
            ...
            await message.channel.send(f"Sorry, but the correct answer was {math_trivia[2]}.")

    if message.content.startswith("$animetrivia"):
        anime_trivia = BotHTTPRequests.get_anime_trivia()
        await message.channel.send(html.unescape(f"{anime_trivia[0]} True or false? The difficulty is {anime_trivia[1]}."))
        ...
        if response.content.lower() == anime_trivia[2].lower():
            ...
            await message.channel.send(f"Sorry, but the correct answer was {anime_trivia[2]}.")
